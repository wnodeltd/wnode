...
func New(dispatcher *jobs.Dispatcher, store *jobs.Store, pricingStore *pricing.Store, accountStore *account.Store, stripeSvc *stripe.Service, moneyHandler *money.Handler, acqHandler *acquisition.Handler, instHandler *institutional.Handler, host *p2p.Host, log *zap.Logger, startTime time.Time) *Server {
	hub := newWSHub()
...
func (s *Server) isOwner(c *fiber.Ctx) bool {
	...
	id := c.Get("X-Owner-ID")
	return email == "stephen@wnode.one" && id == "100001-0426-01-AA"
}
