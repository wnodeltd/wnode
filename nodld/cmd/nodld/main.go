...
func main() {
	...
	accountStore := account.NewStore(forensicsStore)
	...
	founders := []struct {
		...
		{id: "100001-0426-01-AA", email: "stephen@wnode.one", name: "Stephen Soos", status: "active"},
	...
	// ── API Server ────────────────────────────────────────────────────────────
	srv := api.New(dispatcher, store, pricingStore, accountStore, stripeSvc, moneyHandler, acqHandler, instHandler, p2pHost, log, startTime)
